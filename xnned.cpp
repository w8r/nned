    ...
    : QMainWindow(parent)
{
	...
	//QVector<GreyscaleWindow> trainSet;
	//trainSet.push_back( *gsw );
	//trainSet.push_back( *gsw );
		
	//QVector<GreyscaleWindow> detectedImages = net->train( trainSet );
	//detectedImages = net->train( trainSet );
	//net->savePreferences( "train.xml" );
...
void xnned::open()
{
    ...
    if ( !fileName.isEmpty() )
	{
		...
		//out << qGray( image.pixel( 200, 380 ) );
		//GreyscaleWindow *gsw = new GreyscaleWindow( *image );
		
		
		//resultLabel->setPixmap( QPixmap::fromImage( detectedImages[0].toMonochromeImage() ) );
				
		
		//QLabel *s = new QLabel( tr( "%1" ).arg( net->subnetworksCount() ) );
		//s->show();
		//resultLabel->setPixmap( QPixmap::fromImage( gsw->toImage() ) );
		//resultLabel->show();
